...
const petSnapshotSchema = z.object({
  ...
  // rather than the old hardcoded 100 — the client just echoes back whatever it last had.
  health: z.number().int().min(0).max(1000),
  happiness: z.number().int().min(0).max(1000),
  intellect: z.number().int().min(0).max(1000),
  strength: z.number().int().min(0).max(1000),
